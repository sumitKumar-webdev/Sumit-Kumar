...
export function aboutComponent() {
  ...
  return (
    <div
      id="About"
      className="w-full bg-transparent text-white flex items-center justify-center lg:px-28 px-14 flex-col"
    >
      {/* text heading */}
      <div className="lg:mb-28 mb-14 w-full flex relative items-center">
        <h2 className="absolute lg:left-1 font-bold text-2xl lg:text-4xl">
          ABOUT
        </h2>
        <span className="absolute -right-24 lg:-right-40 text-gray-600 font-bold text-4xl lg:text-8xl opacity-50">
          ABOUT
        </span>
      </div>
      ...
      <div className="flex lg:flex-row flex-col items-start justify-between gap-10 w-full">
        <span className="lg:text-left lg:w-1/2 lg:text-lg text-justify tracking-widest leading-relaxed">
          ...
          I enjoy working in collaborative environments and exploring new
          challenges in tech. Outside of coding, I love spending time watching
          movies, business documentaries, and{" "}
          <motion.span
            initial={{ backgroundSize: "0% 100%" }}
            whileInView={{ backgroundSize: "100% 100%" }}
            transition={{ duration: 0.5, ease: "easeOut", delay: 0.4 }}
          ...
          {qualifications.map((item, index) => (
            <motion.div
              ...
              className={`relative bg-white/5 border border-white/10 z-30 rounded-xl p-6 transition-all duration-300 hover:bg-white/10 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] ${
                openSlide === index
                  ? "ring-2 ring-purple-500/50 backdrop-blur-3xl shadow-[0_0_80px_30px_rgba(168,85,247,0.25)]"
                  : ""
              }`}
            >
